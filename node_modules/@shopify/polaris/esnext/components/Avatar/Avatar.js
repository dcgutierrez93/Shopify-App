...
export function Avatar({ name, source, initials, customer, size = 'medium', accessibilityLabel, }) {
    ...
    const className = classNames(styles.Avatar, size && styles[variationName('size', size)], !customer && styles[variationName('style', styleClass(nameString))], (hasImage || (initials && initials.length === 0)) &&
        status !== Status.Loaded &&
        styles.hidden, hasImage && styles.hasImage);
    const imageMarkUp = source && !isServer && status !== Status.Errored ? (<Image className={styles.Image} source={source} alt="" role="presentation" onLoad={handleLoad} onError={handleError}/>) : null;
    // Use `dominant-baseline: central` instead of `dy` when Edge supports it.
    const verticalOffset = '0.35em';
    const avatarBody = customer ? (<g fill="currentColor" fillRule="nonzero">
    ...
    </g>) : (<text x="50%" y="50%" dy={verticalOffset} fill="currentColor" fontSize="20" textAnchor="middle">
      {initials}
    </text>);
    const svgMarkup = !hasImage ? (<span className={styles.Initials}>
